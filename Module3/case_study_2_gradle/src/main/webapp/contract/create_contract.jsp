    ...
    <div class="row">
        ...
        <div class="col-10">
            <form method="post" action="/contract?action=create">
                ...
                <select class="custom-select"name="idEmployee">
                    <option>Select Name Employee</option>
                    <c:forEach var="employee" items="${allEmployee}">
                    <option value="${employee.id}">${employee.name}</option>
                    </c:forEach>
                </select>
                <select class="custom-select"name="idCustomer">
                    <option>Select Name Customer</option>
                    <c:forEach var="customer" items="${allCustomer}">
                        <option value="${customer.id}">${customer.name}</option>
                    </c:forEach>
                </select>
                <select class="custom-select"name="idService">
                    <option>Select Name Service</option>
                    <c:forEach var="service" items="${allService}">
                        <option value="${service.id}">${service.name}</option>
                    </c:forEach>
                </select>
                <input class="form-control" type="date" placeholder="Input day start contract" name="dayStart">
                <input class="form-control" type="date" placeholder="Input  day end contract" name="dayEnd">
                <input class="form-control" type="text" placeholder="Input deposit" name="deposit">
                <input class="form-control" type="text" placeholder="Input total money" name="total">
